...
class Program
{
	static void Main(string[] args)
	{
		...
		const string SensorPort = "COM5";               // Windows format for physical and virtual (USB) serial port.
        ...
        //System.IO.File.WriteAllText(@"C:\Users\ronin\Documents\TestText.txt", "TEST");
        ...
        for (var i = 0; i < 25; i++)
        {
            ...
            else
            {
                ...
                float[] arr = cd.YawPitchRoll.ToArray();
                count++;
                yaw_total += arr[0];
                yaw_avg = yaw_total / count;
                Console.WriteLine("AVERAGE: {0}", yaw_avg);


                using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"C:\Users\ronin\Documents\TestText.txt", true))
                {
                    file.WriteLine(cd.YawPitchRoll);
                }
            }
        }
